#include <stdio.h>
#include <stdlib.h>

typedef struct {
    char iso[10];
    char name[50];
    char consumption_co2_char[10];
    float consumption_co2;
    int year, population;
    float co2, cumulative_co2;
} country;


int main() {
    FILE *file;
    file = fopen("/Users/maelkerichard/CLionProjects/sys-tp-10-11-12/owid-co2-data-excerpt.csv", "r");
    if (file) {
        char line[1024];
        fgets(line, sizeof(line), file); //skip first line
        while (fgets(line, sizeof(line), file)) {
            country c; //create a country struct
            char consumption_co2_char[10]; //create a char array to store the consumption_co2

            sscanf(line, "%[^,],%[^,],%d,%f,%[^,],%f,%d", c.iso, c.name, &c.year, &c.co2, consumption_co2_char,
                   &c.cumulative_co2, &c.population); //extract data from line

            // if consumption_co2 is missing, set it to -1
            c.consumption_co2 = consumption_co2_char[0] == '?' ? -1 : atof(consumption_co2_char);

            if (c.consumption_co2 == -1) {
                printf("No data available on the consumption CO2 caused by %s in %d.\n", c.name,
                       c.year); //print message if consumption_co2 not available
            } else {
                printf("%s caused %f million tons of CO2 in %d.\n", c.name, c.consumption_co2,
                       c.year); //print message if consumption_co2 available
            }

        }
        fclose(file);
    } else {
        printf("File not found");
    }
}